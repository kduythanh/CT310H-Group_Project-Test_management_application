...
ALTER TABLE CAU_HOI
ADD LOAI_NOI_DUNG INT DEFAULT 0;

-- Xóa ràng buộc khóa ngoại cũ (nếu đã tồn tại)
ALTER TABLE CAU_HOI DROP CONSTRAINT FK_CAU_HOI_DE_THI;

-- Thêm lại ràng buộc khóa ngoại với ON DELETE CASCADE
ALTER TABLE CAU_HOI
ADD CONSTRAINT FK_CAU_HOI_DE_THI
FOREIGN KEY (MA_DE_THI) REFERENCES DE_THI(MA_DE_THI) 
ON DELETE CASCADE;
...
DELETE FROM CHI_TIET_BAI_THI WHERE Ma_Bai_Thi = N'48FB978F85';
DELETE FROM BAI_THI WHERE Ma_Bai_Thi = N'48FB978F85';


select * from CHI_TIET_BAI_THI where Ma_Bai_Thi = N'53CE34C8D3';
...
DELETE FROM CHI_TIET_BAI_THI WHERE Ma_Bai_Thi = N'A2225667C4';
DELETE FROM BAI_THI WHERE Ma_Bai_Thi = N'A2225667C4';
...
SELECT * FROM DE_THI DT JOIN MON_HOC MH ON DT.MA_MON = MH.MA_MON;
SELECT * FROM CAU_HOI;
